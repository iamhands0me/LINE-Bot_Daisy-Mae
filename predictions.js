function calculateOutput(buy_input, sell_inputs, first_buy, previous_pattern) {
    const sell_prices = getSellPrices(sell_inputs);
    const buy_price = parseInt(buy_input);
    const prices = [buy_price, buy_price, ...sell_prices];
  let predictor = new Predictor(prices, first_buy, previous_pattern);
  ...
  for (let poss of analyzed_possibilities) {
      return poss.category_total_probability;
  }
}

function getSellPrices(sell_inputs) {
  //Checks all sell inputs and returns an array with their values
    let sells = [];
    for (let i = 0; i < sell_inputs.length; i++) {
        sells.push(parseInt(sell_inputs[i] || ''))
    }
  return sells
}
